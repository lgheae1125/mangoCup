...
function CreateMangoCupPage() {
  ...
  const handleCreateMangoCup = async () => {
    ...
    for (const candidate of candidates) {
      ...
      if (error) {
        console.error("후보 삽입 에러", error.message);
        return;
      }

      console.log("후보 삽입 성공:", candidate.name, "status:", status);
    }
  };

  return (
    <div className="w-full h-full pt-12">
      <section className="w-full h-full bg-white shadow-2xl p-8">
        <h1 className="text-secondary text-2xl font-semibold">망고컵 만들기</h1>
        <article className="mt-8 px-4">
          <p className="ml-1 text-lg text-[#999] font-medium">망고컵 제목</p>
          <input
            type="text"
